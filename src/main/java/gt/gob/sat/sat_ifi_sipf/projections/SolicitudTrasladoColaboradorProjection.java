/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package gt.gob.sat.sat_ifi_sipf.projections;

/**
 *
 * @author ajsbatzmo
 */
public interface SolicitudTrasladoColaboradorProjection {

    Integer getIdSolicitud();

    String getNitColaborador();

    String getNombreColaborador();

    String getNitSolicitante();

    String getNombreAprobador();

    Integer getIdGrupoNuevo();

    String getNombreGrupoNuevo();

    String getNombreUnidadAdmin();

    String getMotivo();

    String getEstado();
    
    String getTipoTraslado();
}
